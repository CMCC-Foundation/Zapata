...
def xmap(data, cont, prol, ax=None, fill=True,contour=True, \
    ...
    #add optionally stippling controlled by logical array "mask" using matplotlib hatches
...
def zonal_plot(data,ax,cont,cmap,colorbar=True, title={}, title_style={},\
                label_style={},\
                refline=None, invert_xaxis=True, invert_yaxis=True, box_linewidth=2):
    """
    ...
    invert_xaxis : bool, optional
            True to invert x axis (default: True)
    invert_yaxis : bool, optional
            True to invert y axis (default: True)
    box_linewidth : float, optional
            Width of the black box around the plot (default: 2)
    ...
    # Apply axis inversion based on parameters
    if invert_yaxis:
        ax.set_ylim(lev[nlev-1], lev[0])  # Invert y axis
    else:
        ax.set_ylim(lev[0], lev[nlev-1])  # Normal y axis
    
    if invert_xaxis:
        ax.set_xlim(90, -90)  # Invert x axis
    else:
        ax.set_xlim(-90, 90)  # Normal x axis
    
    ax.xaxis.set_major_formatter(mticker.FuncFormatter(_update_ticks))
    
    # Add thick black box around the plot
